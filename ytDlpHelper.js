...
export function isValidCookiesFile(filePath) {
  try {
    ...
    if (!fileContent.trim()) {
      console.log(`Cookies file is empty at: ${filePath}`);
      return false;
    }
    
    // Basic validation - check for expected format (Netscape cookies format)
    // Should have a header line and at least one cookie entry
    const lines = fileContent.split('\n').filter(line => line.trim());
    
    if (lines.length < 2) {
      console.log(`Cookies file has insufficient content at: ${filePath}`);
      return false;
    }
    
    // Check for Netscape cookies format header
    if (!lines[0].includes('# Netscape HTTP Cookie File') && 
        !lines[0].includes('# HTTP Cookie File')) {
      console.log(`Cookies file does not have proper header at: ${filePath}`);
      return false;
    }
    
    // Check if there's at least one line that looks like a cookie entry
    // Cookie entries typically have multiple tab-separated fields
    const hasCookieEntry = lines.some(line => {
      // Skip comment lines
      if (line.startsWith('#')) return false;
      // Cookie entries should have multiple tab-separated fields
      return line.includes('\t') && line.split('\t').length >= 6;
    });
    
    if (!hasCookieEntry) {
      console.log(`Cookies file does not contain valid cookie entries at: ${filePath}`);
      return false;
    }
    
    console.log(`Cookies file appears valid at: ${filePath}`);
    return true;
  } catch (error) {
    console.error(`Error validating cookies file at ${filePath}:`, error.message);
    return false;
  }
}
...
export default async function fetchVideoInfo(url) {
  ...
  try {
    ...
    // Log detailed cookies information for debugging
     if (cookiesExist) {
       if (cookiesValid) {
         console.log(`Using valid cookies file: ${cookiesPath}`);
         try {
           const cookieStats = fs.statSync(cookiesPath);
           console.log(`Cookies file size: ${cookieStats.size} bytes`);
           console.log(`Cookies file last modified: ${cookieStats.mtime}`);
           console.log(`Cookies file permissions: ${cookieStats.mode.toString(8)}`);
         } catch (statError) {
           console.error(`Error getting cookies file stats: ${statError.message}`);
         }
       } else {
         console.log(`Found cookies file at ${cookiesPath} but it appears to be invalid. Not using it.`);
       }
     } else {
       console.log('No cookies file found. Checked locations:');
       console.log('- ./youtube-cookies.txt');
       console.log('- /youtube-cookies.txt');
       console.log('- /etc/secrets/youtube-cookies.txt');
     }
    ...
    if (err.message.includes("Video unavailable") || (err.stderr && err.stderr.includes("Video unavailable"))) {
      ...
      // Check if we tried with valid cookies
       if (cookiesExist && cookiesValid) {
         errorMessage = "This video requires authentication. The cookies file might be expired or insufficient for this video.";
         console.log("Authentication error despite using valid cookies file. Cookie file might be expired or insufficient.");
       } else if (cookiesExist && !cookiesValid) {
         errorMessage = "This video requires authentication. The cookies file is invalid. Please upload a valid youtube-cookies.txt file.";
         console.log("Error occurred with invalid cookies file.");
       } else {
         errorMessage = "This video requires authentication. Please upload a valid youtube-cookies.txt file to /etc/secrets/ or the root directory.";
         console.log("No cookies file found. Authentication required.");
       }
